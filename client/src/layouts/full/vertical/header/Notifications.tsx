...
const Notifications = () => {
  ...
  const { total_notification } = useSelector((state: any) => state.notifications);
  ...
  useEffect(() => {
    const fetchnotification = async () => {
      try {
        await dispatch(TotalNotification()).unwrap();
      } catch (error) {
  ...
  const getFilteredNotifications = () => {
    ...
    if (keyword) {
      return notificationList.filter((n) => n.title?.toLowerCase().includes(keyword));
    }
    ...
    <div className="relative group/menu">
        ...
        renderTrigger={() => (
          <div className="relative">
            <span className="h-10 w-10 hover:bg-lightprimary rounded-full flex justify-center items-center cursor-pointer group-hover/menu:bg-lightprimary group-hover/menu:text-primary">
              <Icon icon="solar:bell-bing-line-duotone" height={20} onClick={handleShowNoti} />
            </span>
      ...
      >
        <div className="flex items-center px-6 justify-between">
          ...
          <Badge color={'primary'}>{total_notification || 0}</Badge>
        </div>
