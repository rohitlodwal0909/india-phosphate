...
const DispatchInventoryTable = () => {
  ...
  const ProductionAlldata = useSelector((state: RootState) => state.productionData.productiondata)as any;
  ...
  const filtered = qcAlldata.data
    .filter(i => batchIds.includes(String(i.id)));
  ...
  const filteredData = useMemo(() => data.filter(item => Object.values(item).some(v => String(v || '').toLowerCase().includes(searchText.toLowerCase()))), [data, searchText]);
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor("quantity", {
      header: "Quantity",
      cell: info => <span>{info.getValue()} {info.row.original.unit || ""}</span>
    }),
    columnHelper.accessor("batch_numbers", {
      header: "Batch Numbers",
      cell: info => {
        const val = typeof info.row.original.batch_numbers === 'string' ? JSON.parse(info.row.original.batch_numbers || '[]') : info.row.original.batch_numbers;
      ...
      cell: info => {
        ...
        return (
          <div className="flex gap-2 notranslate" translate="no">
            {permissions.view && <Tooltip content="View"><Button onClick={() => handleModal("view", true, row)} color="primary" outline size="xs" className="text-primary bg-lightprimary hover:text-white"><Icon icon="solar:eye-outline" height={18} /></Button></Tooltip>}
            {permissions.edit && <Tooltip content="Edit"><Button size="sm" className="p-0 bg-lightsuccess text-success hover:bg-success hover:text-white" onClick={() => handleModal("edit", true, row)}><Icon icon="solar:pen-outline" height={18} /></Button></Tooltip>}
            {permissions.del && <Tooltip content="Delete"><Button size="sm" color="lighterror" className="p-0" onClick={() => handleModal("delete", true, row)}><Icon icon="solar:trash-bin-minimalistic-outline" height={18} /></Button></Tooltip>}
          </div>
        )
      }
    })
  ...
  return (
    <div className="p-1">
      <div className="flex justify-end mb-2">
        {permissions.view && <input type="text" placeholder="Search..." value={searchText} onChange={e => setSearchText(e.target.value)} className="me-2 p-2 border rounded-md border-gray-300" />}
        {permissions.add && <Button onClick={() => handleModal("add", true)} color="primary" outline size="sm" className="border border-primary bg-primary text-white rounded-md">Add Dispatch Entry</Button>}
      </div>
      {permissions.view ? (
        <>
