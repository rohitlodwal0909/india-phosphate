...
const Addproductionmodal: React.FC<VehicleDispatchModalProps> = ({
  openModal,
  setOpenModal,
  rmcode,
  selectedRow,
  ...
  const handleSubmit = async (e) => {
    ...
    const hasEmpty = formData.items.some((item: any) => !item.rm_code || !item.quantity);
    if (hasEmpty) newErrors.items = 'All RM rows must have RM Code and Quantity';
    ...
    <Modal show={openModal} onClose={() => setOpenModal(false)}>
      ...
      <Modal.Body>
        <form onSubmit={handleSubmit} className="grid grid-cols-12 gap-4">
            ...
            <div key={index} className="col-span-12 grid grid-cols-12 gap-4 items-end">
              <div className="col-span-5">
                ...
                <Select
                  ...
                  onChange={(selected) => handleItemChange(index, 'rm_code', selected?.value || '')}
                  className="w-full"
              ...
              <div className="col-span-2">
                {index == 0 ? (
                  ...
                  <Button type="button" color="error" onClick={() => handleDeleteRow(index)}>
          ...
          {errors.items && <p className="text-red-500 text-sm col-span-12">{errors.items}</p>}
