...
const CompanyTable = () => {
  ...
  const { Companydata, loading } = useSelector((state: any) => state.company);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteCompany({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetCompany());
      toast.success("The company was successfully deleted.");
    } catch (error: any) {
  ...
  const filteredItems = (Companydata || []).filter((item: any) => {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              {["Sr.No", "Company Name", "Email ", "Phone  ", "Address", "GST Number",  "Status", "Action"].map((title) => (
                <th
                  key={title}
