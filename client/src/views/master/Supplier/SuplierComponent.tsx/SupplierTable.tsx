...
import { deleteSupplier, GetSupplier } from 'src/features/master/Supplier/SupplierSlice';
...
const SupplierTable = () => {
  ...
  const { supplierdata, loading } = useSelector((state: any) => state.supplier);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(
        deleteSupplier({ id: userToDelete?.id, user_id: logindata?.admin?.id }),
      ).unwrap();
      dispatch(GetSupplier());
      toast.success('The supplier was successfully deleted.');
    } catch (error: any) {
  ...
  const filteredItems = (supplierdata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const supllier = item?.supplier_name || '';
    ...
    <div>
      ...
      <div className="flex justify-end mb-3 gap-2">
        ...
        {permissions?.add && (
          ...
          >
            Create Supplier {/* <Icon icon="ic:baseline-plus" height={18} /> */}
          </Button>
        )}
      </div>
      {permissions?.view ? (
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.supplier_name || '-').replace(/^\w/, (c: string) =>
                          c.toUpperCase(),
                        )}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
