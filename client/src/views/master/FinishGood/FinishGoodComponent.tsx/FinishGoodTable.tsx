...
import EditFinishGoodModal from "./EditFinishGoodModal";
import AddFinishGoodModal from "./AddFinishGoodModal";
import { deleteFinishGood, GetFinishGood } from "src/features/master/FinishGood/FinishGoodSlice";
...
const FinishGoodTable = () => {
  ...
  const { FinishGooddata, loading } = useSelector((state: any) => state.finishgood);
  ...
  const [searchTerm, setSearchTerm] = useState("");
   const { selectedIconId } = useContext(CustomizerContext) || {};
                      ...
                      return getPermissions(logindata, selectedIconId, 28);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteFinishGood({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetFinishGood());
      toast.success("The finish good was successfully deleted.");
    } catch (error: any) {
...
const filteredItems = (FinishGooddata || []).filter((item: any) => {
  const searchText = searchTerm.toLowerCase();
  return (
    String(item?.product_code)?.toLowerCase().includes(searchText) ||
    ...
    String(item?.packing_details)?.toLowerCase().includes(searchText) ||
    String(item?.hsn_code)?.toLowerCase().includes(searchText) ||
    String(item?.gst)?.toLowerCase().includes(searchText) ||
    String(item?.storage_condition)?.toLowerCase().includes(searchText) ||
    String(item?.mrp)?.toLowerCase().includes(searchText)
  );
});
  ...
  return (
    ...
    { permissions?.view ? <>  <div className="overflow-x-auto">
        ...
        "Packing Details", "HSN Code", "GST", "Storage Condition", "MRP",  "Description","Action"].map((title) => (
          ...
          <td className="py-3 px-4 text-gray-900 dark:text-gray-300">{item?.hsn_code || "-"}</td>
          <td className="py-3 px-4 text-gray-900 dark:text-gray-300">{item?.gst_rate || "-"}</td>
      ...
      <AddFinishGoodModal setShowmodal={setAddmodal} show={addmodal}  logindata={logindata}/>
      <EditFinishGoodModal show={editmodal} setShowmodal={setEditmodal} FinishGoodData={selectedrow} logindata={logindata} />
    </div>
  );
};
