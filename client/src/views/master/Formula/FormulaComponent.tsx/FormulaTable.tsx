...
import FormulaUnitModal from './FormulaUnitModal';
...
const FormulaTable = () => {
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(
        deleteFormula({ id: userToDelete?.id, user_id: logindata?.admin?.id }),
      ).unwrap();
      dispatch(GetFormula());
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {item?.quantity_per_batch_or_unit || '-'}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {item?.uom || '-'}
                      </td>
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {item?.manufacturing_instructions || '-'}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {item?.remarks || '-'}
                      </td> */}
