...
const CurrencyTable = () => {
  ...
  const { Currencydata, loading } = useSelector((state: any) => state.currency);
...
const { selectedIconId } = useContext(CustomizerContext) || {};
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteCurrency({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetCurrency());
      toast.success("The currency was successfully deleted.");
    } catch (error: any) {
  ...
  return (
    ...
    {permissions?.view ?  <><div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
            {item?.country || "-"}
                  ...
                  <td className="py-3 px-4 text-gray-900 dark:text-gray-300"> 
                           <Badge color={ item.is_active   ? `lightprimary` :"lighterror"}
      ...
      <AddCurrencyModal setShowmodal={setAddmodal} show={addmodal}  logindata={logindata} />
      <EditCurrencyModal show={editmodal} setShowmodal={setEditmodal} CurrencyData={selectedrow} logindata={logindata} />
    </div>
  );
};
