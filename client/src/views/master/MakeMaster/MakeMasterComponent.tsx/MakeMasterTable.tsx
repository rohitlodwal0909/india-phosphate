...
const MakeMasterTable = () => {
  ...
  const { MakeMasterdata, loading } = useSelector((state: any) => state.makemaster);
  ...
  const [searchTerm, setSearchTerm] = useState("");
   const { selectedIconId } = useContext(CustomizerContext) || {};
                  ...
                  return getPermissions(logindata, selectedIconId, 19);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteMakeMaster({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetMakeMaster());
      toast.success("The make master was successfully deleted.");
    } catch (error: any) {
  ...
  return (
    <div>
      ...
      { permissions?.add && <Button size="sm" className="p-0 bg-primary border rounded-md"   onClick={() => { setAddmodal(true); }}  >
         Create Make master  {/* <Icon icon="ic:baseline-plus" height={18} /> */}
        </Button>}
      </div>
  ...
  { permissions?.view ?    <> <div className="overflow-x-auto">
      ...
      <EditMakeMasterModal show={editmodal} setShowmodal={setEditmodal} MakeMasterData={selectedrow} logindata={logindata} />
    </div>
  );
};
