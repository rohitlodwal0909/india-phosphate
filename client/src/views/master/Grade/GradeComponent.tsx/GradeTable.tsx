...
import { deleteGrade, GetGrade } from 'src/features/master/Grade/GradeSlice';
...
const GradeTable = () => {
  ...
  const { gradedata, loading } = useSelector((state: any) => state.grades);
  ...
  const permissions = useMemo(() => {
    return getPermissions(logindata, selectedIconId, 35);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteGrade({ id: userToDelete?.id })).unwrap();
      dispatch(GetGrade());
      toast.success('The grade was successfully deleted.');
    } catch (error: any) {
  ...
  const filteredItems = (gradedata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const supllier = item?.grade || '';
    ...
    <div>
      ...
      <div className="flex justify-end mb-3 gap-2">
        ...
        {permissions?.add && (
          ...
          >
            Create Grade {/* <Icon icon="ic:baseline-plus" height={18} /> */}
          </Button>
        )}
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  {['Sr.No', 'Grade Name', 'Action'].map((title) => (
                    <th
                      key={title}
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.grade || '-').replace(/^\w/, (c: string) => c.toUpperCase())}
                      </td>
      ...
      <AddGradeModal setShowmodal={setAddmodal} show={addmodal} />
      <EditGradeModal show={editmodal} setShowmodal={setEditmodal} GradeData={selectedrow} />
    </div>
  );
};
