...
import { deleteQualification, GetQualification } from "src/features/master/Qualification/QualificationSlice";
...
const QualificationTable = () => {
  ...
  const { Qualificationdata, loading } = useSelector((state: any) => state.qualification);
  ...
  const [searchTerm, setSearchTerm] = useState("");
   const { selectedIconId } = useContext(CustomizerContext) || {};
                  ...
                  return getPermissions(logindata, selectedIconId, 18);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteQualification({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetQualification());
      toast.success("The qualification was successfully deleted.");
    } catch (error: any) {
  ...
  return (
    <div>
      ...
      { permissions?.add &&  <Button size="sm" className="p-0 bg-primary border rounded-md"   onClick={() => { setAddmodal(true); }}  >
         Create Qualification  {/* <Icon icon="ic:baseline-plus" height={18} /> */}
        </Button>}
      </div>
    ...
    {permissions?.view ?  <> <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              {["Sr.No", "Qualification Name", "Status","Action"].map((title) => (
                <th
                  key={title}
