...
import { deleteCategory, GetCategory } from "src/features/master/Category/CategorySlice";
...
const CategoryTable = () => {
  ...
  const { categorydata, loading } = useSelector((state: any) => state.category);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteCategory({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetCategory());
  ...
  const filteredItems = (categorydata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const supllier = item?.category_name || "";
    ...
    <div>
     ...
     { permissions?.view &&   <input
        ...
        {  permissions?.add &&  <Button size="sm" className="p-0 bg-primary border rounded-md"   onClick={() => { setAddmodal(true); }}  >
         Create Category  {/* <Icon icon="ic:baseline-plus" height={18} /> */}
        </Button>}
      </div>
 ...
 { permissions?.view ?<>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              {["Sr.No", "Category Name", "Action"].map((title) => (
                <th
                  key={title}
      ...
      <AddCategoryModal setShowmodal={setAddmodal} show={addmodal}  logindata={logindata} />
      <EditCategoryModal show={editmodal} setShowmodal={setEditmodal} CategoryData={selectedrow} logindata={logindata} />
    </div>
  );
};
