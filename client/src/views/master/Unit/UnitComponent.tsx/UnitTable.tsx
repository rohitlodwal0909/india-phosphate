...
import { deleteUnit, GetUnit } from "src/features/master/Unit/UnitSlice";
...
const UnitTable = () => {
  ...
  const { unitdata, loading } = useSelector((state: any) => state.unit);
  ...
  const [searchTerm, setSearchTerm] = useState("");
       const { selectedIconId } = useContext(CustomizerContext) || {};
                  ...
                  return getPermissions(logindata, selectedIconId, 16);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteUnit({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetUnit());
  ...
  const filteredItems = (unitdata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const supllier = item?.unit || "";
    return (
    ...
    <div>
     ...
     {permissions?.add && <div className="flex justify-end mb-3 gap-2">
        ...
        <Button size="sm" className="p-0 bg-primary border rounded-md"   onClick={() => { setAddmodal(true); }}  >
         Create Unit  {/* <Icon icon="ic:baseline-plus" height={18} /> */}
        </Button>
      </div>}
     ...
     <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              {["Sr.No", "Unit Name", "Action"].map((title) => (
                <th
                  key={title}
      ...
      <AddUnitModal setShowmodal={setAddmodal} show={addmodal}  logindata={logindata} />
      <EditUnitModal show={editmodal} setShowmodal={setEditmodal} UnitData={selectedrow} logindata={logindata} />
    </div>
  );
};
