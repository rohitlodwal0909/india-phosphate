...
import AddPendingOrderModal from "./AddPendingOrderModal";
import { deletePendingOrder, GetPendingOrder } from "src/features/master/PendingOrder/PendingOrderSlice";
...
const PendingOrderTable = () => {
  ...
  const { PendingOrderdata, loading } = useSelector((state: any) => state.pendingorders);
  ...
  const filteredItems = (PendingOrderdata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const supllier = item?.order_number || "";
    const mouldNo = item?.customer_name_or_id || "";
    const hardness = item?.products_ordered || "";
    const temperature = item?.quantity_pending || "";
  ...
  return (
    <div>
     ...
     {permissions?.add &&     <Button size="sm" className="p-0 bg-primary border rounded-md"   onClick={() => { setAddmodal(true); }}  >
         Create Pending Order  {/* <Icon icon="ic:baseline-plus" height={18} /> */}
        </Button>}
      </div>
    ...
    {loading ? (
      ...
      currentItems.map((item: any, index: number) => (
        <tr key={item.id} className="bg-white dark:bg-gray-900">
          ...
          <td className="py-3 px-4 text-gray-900 dark:text-gray-300">{item?.order_number || "-"}</td>
      ...
      <ComonDeletemodal
        ...
        title="Are you sure you want to Delete this Pending Order?"
      />
       <ViewPendingOrderModal setPlaceModal={setViewModal} modalPlacement={"center"} selectedRow={selectedrow} placeModal={viewModal} />
      <AddPendingOrderModal setShowmodal={setAddmodal} show={addmodal}  logindata={logindata}  rmcodedata={rmcodedata} />
      <EditPendingOrderModal show={editmodal} setShowmodal={setEditmodal} PendingOrderData={selectedrow} logindata={logindata} rmcodedata={rmcodedata} />
    </div>
  );
};
