...
import { deleteDesignation, GetDesignation } from "src/features/master/Designation/DesignationSlice";
...
const DesignationTable = () => {
  ...
  const { Designationdata, loading } = useSelector((state: any) => state.designation);
  ...
  const [searchTerm, setSearchTerm] = useState("");
   const { selectedIconId } = useContext(CustomizerContext) || {};
                  ...
                  return getPermissions(logindata, selectedIconId, 17);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteDesignation({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetDesignation());
      toast.success("The designation was successfully deleted.");
    } catch (error: any) {
  ...
  const filteredItems = (Designationdata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const supllier = item?.Designation_name || "";
    ...
    { permissions?.view ? <><div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              {["Sr.No", "Designation Name","Status", "Action"].map((title) => (
                <th
                  key={title}
