...
import EditStockMasterModal from "./EditStockMasterModal";
import AddStockMasterModal from "./AddStockMasterModal";
import { deleteStockMaster, GetStockMaster } from "src/features/master/StockMaster/StockMasterSlice";
import { GetCity } from "src/features/master/City/CitySlice";
import ViewStockMasterModal from "./ViewStockMasterModal";
...
const StockMasterTable = () => {
  const logindata = useSelector((state: any) => state.authentication?.logindata);
  const dispatch = useDispatch<AppDispatch>();
  const { StockMasterdata, loading } = useSelector((state: any) => state.stockmasters);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteStockMaster({id:userToDelete?.stock_id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetStockMaster());
  ...
  const filteredItems = (StockMasterdata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const supllier = item?.batch_no || "";
    const mouldNo = item?.item_type || "";
    const hardness = item?.quantity_in_stock || "";
  ...
  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
    {loading ? (
      ...
      <EditStockMasterModal show={editmodal} setShowmodal={setEditmodal} StockMasterData={selectedrow} logindata={logindata} itemList={rmcodedata} locationList={locationList} batchnumber={qcAlldata}/>
    </div>
  );
};
