...
const OutwardTable = () => {
  ...
  const { Outwarddata, loading } = useSelector((state: any) => state.outward);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteOutward({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetOutward());
      toast.success("The outward was successfully deleted.");
    } catch (error: any) {
  ...
  const filteredItems = (Outwarddata || []).filter((item: any) => {
  const searchText = searchTerm.toLowerCase();
  return (
    String(item?.outword_number)?.toString().toLowerCase().includes(searchText) ||
    ...
    String(item?.outword_date)?.toLowerCase().includes(searchText) ||
  ...
  return (
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              {["Sr.No", "Outward Number", "Outward Date", "Vendor", "Item","Quantity", "Unit", "Vehicle Number","Action"].map((title) => (
                <th
                  key={title}
      ...
      <AddOutwardModal setShowmodal={setAddmodal} show={addmodal}  logindata={logindata} vendordata={supplierdata}/>
      <EditOutwardModal show={editmodal} setShowmodal={setEditmodal} OutwardData={selectedrow} logindata={logindata} vendordata={supplierdata} />
    </div>
  );
};
