...
const TransportTable = () => {
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(
        deleteTransport({ id: userToDelete?.transporter_id, user_id: logindata?.admin?.id }),
      ).unwrap();
      dispatch(GetTransport());
        ...
        <>
          <div className="overflow-x-auto">
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.transporter_name || '-').replace(/^\w/, (c: string) =>
                          c.toUpperCase(),
                        )}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
            ...
            <td className="py-3 px-4 text-gray-900 dark:text-gray-300">{item?.state || "-"}</td> */}
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        <Badge
                          color={item.is_active == 1 ? `lightprimary` : 'lightwarning'}
