...
const qcrowmaterial = useSelector((state: any) => state.qcinventory.qcdata);
...
useEffect(() => {
    ...
    const fetchrawmaterial = async () => {
        try {
            const rm_code = selectedRow?.rm_code
          await dispatch(GetrawMaterial(rm_code)).unwrap(); // throws on error
        } catch (error) {
...
>
  ...
  <ModalBody>
     ...
     <Tabs aria-label="Tabs with underline" variant="underline">
          ...
          <TabItem title="Raw Material " icon={() => <Icon icon="solar:graph-linear" height={20} />}>
      ...
      <tbody>
        {showFollowup.map((item, index) => (
          <tr key={index} className="hover:bg-gray-50">
            ...
            <td className="px-4 py-2 border break-words">{item?.rm_code}</td>
            <td className="px-4 py-2 border capitalize">{item.type == 1 ?"Raw Material": "Physical Properties"}</td>
