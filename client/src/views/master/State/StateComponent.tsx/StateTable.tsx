...
import { deleteState, GetState } from 'src/features/master/State/StateSlice';
...
const StateTable = () => {
  ...
  const { Statedata, loading } = useSelector((state: any) => state.states);
  ...
  const permissions = useMemo(() => {
    return getPermissions(logindata, selectedIconId, 22);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteState({ id: userToDelete?.id, user_id: logindata?.admin?.id })).unwrap();
      dispatch(GetState());
      toast.success('The state was successfully deleted.');
    } catch (error: any) {
  ...
  const filteredItems = (Statedata || []).filter((item: any) => {
    const searchText = searchTerm.toLowerCase();
    const supllier = item?.State_name || '';
    ...
    <div>
      ...
      <div className="flex justify-end mb-3 gap-2">
        ...
        {permissions?.add && (
          ...
          >
            Create State {/* <Icon icon="ic:baseline-plus" height={18} /> */}
          </Button>
        )}
      </div>
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.state_name || '-').replace(/^\w/, (c: string) => c.toUpperCase())}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.code || '-').replace(/^\w/, (c: string) => c.toUpperCase())}
                      </td>
