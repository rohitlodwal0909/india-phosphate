...
import EditCustomerModal from './EditOppertunityModal';
import AddCustomerModal from './AddOpportunityModal';
...
import { getPotOppertunity } from 'src/features/master/Customer/PotentialOpportunitySlice';

const OpportunityTable = () => {
  const logindata = useSelector((state: any) => state.authentication?.logindata);
  const dispatch = useDispatch<AppDispatch>();

  const { potoppertunity, loading } = useSelector((state: any) => state.potentialopportunity);
  ...
  const permissions = useMemo(() => {
    return getPermissions(logindata, selectedIconId, 36);
  ...
  const filteredItems = (potoppertunity || []).filter((item: any) => {
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.company_name || '-').replace(/^\w/, (c: string) => c.toUpperCase())}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {item?.customer_type || '-'}
                      </td>
                      ...
                      <td>
                        <div className="max-w-[350px] whitespace-normal text-sm space-y-1">
                          ...
                          ) : typeof item?.products === 'string' && item?.products ? (
                            JSON.parse(item.products)?.map((c: any, index: number) => (
                              <div key={index} className="border-b pb-1">
                                <p>
                                  <strong>Product:</strong> {c.product}
                                </p>
