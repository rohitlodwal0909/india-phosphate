...
const PackingMaterialTable = () => {
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(
        deletePackingMaterial({ id: userToDelete?.pm_id, user_id: logindata?.admin?.id }),
      ).unwrap();
      dispatch(GetPackingMaterial());
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.material_name || '-').replace(/^\w/, (c: string) =>
                          c.toUpperCase(),
                        )}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {item?.supplier_id
                          ? supplierdata?.find((id) => id.id == item?.supplier_id)?.supplier_name
                          : '-'}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {item?.purchase_rate || '-'}
  ...
  <td className="py-3 px-4 text-gray-900 dark:text-gray-300">{item?.state || "-"}</td> */}
      ...
      />
      <AddPackingMaterialModal
        setShowmodal={setAddmodal}
        show={addmodal}
        logindata={logindata}
        unitOptions={allUnits}
      />
      <EditPackingMaterialModal
        show={editmodal}
        setShowmodal={setEditmodal}
        PackingMaterialData={selectedrow}
        logindata={logindata}
        unitOptions={allUnits}
      />
    </div>
