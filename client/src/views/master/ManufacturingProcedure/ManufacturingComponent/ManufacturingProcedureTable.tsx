...
import AddManufacturingModal from './AddManufacturingModal';
import EditManufacturingModal from './EditManufacturingModal';
import {
  deleteProcedure,
  GetProcedure,
...
const ManufacturingProcedureTable = () => {
  ...
  const { data, loading } = useSelector((state: any) => state.procedure);
  ...
  const permissions = useMemo(() => {
    return getPermissions(logindata, selectedIconId, 31);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deleteProcedure({ id: userToDelete?.id })).unwrap();
      dispatch(GetProcedure());
      toast.success('The procedure was successfully deleted.');
    } catch (error: any) {
  ...
  const filteredItems = (data || []).filter((item: any) => {
    ...
    return String(item?.outword_number)?.toString().toLowerCase().includes(searchText);
  });
    ...
    <div>
      ...
      <div className="flex justify-end mb-3 gap-2">
        ...
        {permissions?.add && (
          ...
          >
            Create Procedure <Icon icon="ic:baseline-plus" height={18} />
          </Button>
        )}
      </div>
...
export default ManufacturingProcedureTable;
