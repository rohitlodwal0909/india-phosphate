...
import { deletePurchase, GetPurchase } from "src/features/master/Purchase/PurchaseSlice";
...
const PurchaseTable = () => {
  ...
  const { Purchasedata, loading } = useSelector((state: any) => state.purchase);
 ...
 const { selectedIconId } = useContext(CustomizerContext) || {};
                    ...
                    return getPermissions(logindata, selectedIconId, 26);
  ...
  const handleDelete = async (userToDelete: any) => {
    ...
    try {
      await dispatch(deletePurchase({id:userToDelete?.id ,user_id:logindata?.admin?.id})).unwrap();
      dispatch(GetPurchase());
      toast.success("The purchase was successfully deleted.");
    } catch (error: any) {
  ...
  const filteredItems = (Purchasedata || []).filter((item: any) => {
  const searchText = searchTerm.toLowerCase();
  return (
   String(item?.purchase_number)?.toString().toLowerCase().includes(searchText) ||
   String(item?.unit)?.toLowerCase().includes(searchText) ||
   String( item?.vendor_id)?.toLowerCase().includes(searchText) ||
   String( item?.price)?.toLowerCase().includes(searchText) ||
   String( item?.item)?.toLowerCase().includes(searchText) ||
   String( item?.purchase_date)?.toLowerCase().includes(searchText) ||
  ...
  return (
    <div>
      ...
      {permissions?.add &&  <Button size="sm" className="p-0 bg-primary border rounded-md"   onClick={() => { setAddmodal(true); }}  >
         Create Purchase  {/* <Icon icon="ic:baseline-plus" height={18} /> */}
        </Button>}
      </div>
     ...
     { permissions?.view ? <><div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              {["Sr.No", "Purchase Number", "Purchase Date", "Vendor", "Item","Quantity", "UOM", "Price", "Remarks","Action"].map((title) => (
                <th
                  key={title}
      ...
      <AddPurchaseModal setShowmodal={setAddmodal} show={addmodal}  logindata={logindata} vendordata={supplierdata}/>
      <EditPurchaseModal show={editmodal} setShowmodal={setEditmodal} PurchaseData={selectedrow} logindata={logindata} vendordata={supplierdata} />
    </div>
  );
};
