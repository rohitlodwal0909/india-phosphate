...
import {
  updateDocument,
  GetDocument,
} from 'src/features/master/Documents/DocumentSlice';

const EditDocumentModal = ({ show, setShowmodal, DocumentData, logindata ,customerdata}) => {
  ...
  const validateForm = () => {
    const required = ['export_type', 'customer_name'];
    const newErrors: any = {};
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const result = await dispatch(updateDocument(formData)).unwrap();
  ...
  return (
    <Modal show={show} onClose={() => setShowmodal(false)} size="2xl">
      <ModalHeader>Edit Export Document</ModalHeader>
      <ModalBody>
