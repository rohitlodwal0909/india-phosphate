...
import {
  ...
  downloadDraftPackingList,
  getDraftPackingList,
} from 'src/features/account/exportinvoice/draft';

const DraftTable = () => {
  const logindata = useSelector((state: any) => state.authentication?.logindata);
  const { draftPackingLists, loading } = useSelector(
    (state: RootState) => state.draftPackingList,
  ) as any;
  ...
  // 🔍 Search Filter
  const filteredItems = (draftPackingLists || []).filter((item: any) => {
  ...
  const handleDelete = async () => {
    await dispatch(deleteDraftPackingList(selectedRow?.id));
    toast.success('Draft packing list deleted successfully');

    setDeleteModel(false);
  };

  const handleDownload = (id: number) => {
    dispatch(downloadDraftPackingList(id));
  };
