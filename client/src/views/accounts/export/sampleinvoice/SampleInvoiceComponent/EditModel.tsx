...
const EditModel = ({ placeModal, setPlaceModal, editData }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('Sample invoice updated successfully');

      await dispatch(getSampleInvoice());
    ...
    <Modal show={placeModal} size="2xl" popup position="center" onClose={handleClose}>
      ...
      <ModalBody>
        <div className="space-y-6 py-2">
          {/* Invoice Name */}
          <div>
            <Label value="Sample Invoice Name" className="mb-2 block font-medium" />
            <TextInput
              value={formData.name}
              onChange={handleChange}
              placeholder="Enter sample invoice name"
              sizing="lg"
            />
          </div>
