...
import {
  ...
  getSampleInvoice,
} from 'src/features/account/exportinvoice/sampleinvoice';

const SampleInvoiceTable = () => {
  const logindata = useSelector((state: any) => state.authentication?.logindata);
  const { invoices, loading } = useSelector((state: RootState) => state.sampleInvoice) as any;
  ...
  const handleDelete = async () => {
    await dispatch(deleteSampleInvoice(selectedRow?.id));
  ...
  const handleDownload = (id: number) => {
    dispatch(downloadSampleInvoice(id));
  };
...
export default SampleInvoiceTable;
