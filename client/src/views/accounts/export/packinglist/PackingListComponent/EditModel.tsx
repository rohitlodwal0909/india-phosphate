...
import { getPackingList, updatePackingList } from 'src/features/account/exportinvoice/packinglist';
...
const EditModel = ({ placeModal, setPlaceModal, editData }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('Package list updated successfully');

      await dispatch(getPackingList());
    ...
    <Modal show={placeModal} size="2xl" popup position="center" onClose={handleClose}>
      ...
      <ModalBody>
        <div className="space-y-6 py-2">
          {/* Invoice Name */}
          <div>
            <Label value="Invoice Name" className="mb-2 block font-medium" />
            <TextInput
              value={formData.name}
