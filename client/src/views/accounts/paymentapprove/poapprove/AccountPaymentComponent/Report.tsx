...
import { getTestReport } from 'src/features/Inventorymodule/Qcinventorymodule/QcinventorySlice';
...
const ViewReport = () => {
  ...
  const { testReports } = useSelector((state: any) => state.qcinventory);

  const Rawmaterialrmcode = testReports?.tests || [];
  const grn = testReports?.grn || {};
  ...
  const getResultValue = (item: any) => {
    ...
    return grn?.qa_qc_status === 'APPROVED' ? results[0]?.result_value : 'fail';
  };
        ...
        >
          <div className="flex justify-between items-start">
            ...
            <div className="text-center text-black">
              ...
              <h2 className="font-bold text-base mt-4 underline">
                {' '}
                {isPM ? 'PACKING' : 'RAW'} MATERIAL TEST REPORT
              </h2>
            </div>
          ...
          <div className="text-sm text-black my-3">
            <div className="grid grid-cols-12 border border-black">
              ...
              <div className="col-span-2 border-r border-black p-1">
                {isPM ? grn?.pm_code?.name : grn?.rmcode?.rm_code}
              </div>
              <div className="col-span-2 font-semibold border-r border-black p-1">
                Date of Receipt
              </div>
            ...
            <table className="w-full text-sm border border-black mt-1 text-black">
              ...
              <tbody>
                {type2Tests.map((item, index) => {
                  ...
                  const resultValue =
                    hasQcResults && grn?.qa_qc_status === 'APPROVED' ? result : 'fail';
          ...
          <p className="mt-8 text-sm text-black ">
            ...
            <strong className="inline-block border-b border-black w-64 ps-3">
              {grn?.qa_qc_status === 'APPROVED' ? 'Complies' : 'Not Complies'}
            </strong>
          </p>
