...
const QuotationTable = () => {
  ...
  const { quotations, loading } = useSelector((state: RootState) => state.quotation) as any;
  ...
  const handleDelete = async (selectedrow: any) => {
    ...
    try {
      await dispatch(deleteQuotation(selectedrow?.id)).unwrap();
      dispatch(getQuotation());
  ...
  const filteredItems = (quotations || []).filter((item: any) => {
    ...
    <div>
      ...
      <div className="flex justify-end mb-3 gap-2">
        ...
        {permissions?.add && (
          ...
          >
            Create Quotation <Icon icon="ic:baseline-plus" height={18} />
          </Button>
        )}
      </div>
      {permissions?.view ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  {['Sr.No', 'Company Name', 'Person Name', 'Contact No.', 'Date', 'Action'].map(
                    (title) => (
                      <th
                        key={title}
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.contact_person || '-').replace(/^\w/, (c: string) =>
                          c.toUpperCase(),
                        )}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
                        {(item?.mobile || '-').replace(/^\w/, (c: string) => c.toUpperCase())}
                      </td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">
