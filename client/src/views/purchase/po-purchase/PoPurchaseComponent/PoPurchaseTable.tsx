...
import CreateModel from './AddPoPurchase';
import EditModel from './EditPoRequisition';
import ViewModal from './ViewPoPurchase';
import ComonDeletemodal from '../../../../utils/deletemodal/ComonDeletemodal';
import { toast } from 'react-toastify';
import { deletePurchasePo, getPurchasePo } from 'src/features/purchase/po/PurchasePoSlice';
...
const PoPurchaseTable = () => {
  const logindata = useSelector((state: any) => state.authentication?.logindata);
  const { purchasepos, loading } = useSelector((state: RootState) => state.purchasepo) as any;
  ...
  const handleDelete = async (selectedrow: any) => {
    ...
    try {
      await dispatch(deletePurchasePo(selectedrow?.id)).unwrap();
      dispatch(getPurchasePo());
      toast.success('Po Purchase deleted.');
    } catch (error: any) {
  ...
  const filteredItems = (purchasepos || []).filter((item: any) => {
  ...
  return (
    <div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  {['Sr.No', 'Date', 'PO No', 'Bill To', 'Expected Arrival Date', 'Action'].map(
                    (title) => (
                      <th
                        key={title}
                ...
                {loading ? (
                  ...
                  currentItems.map((item: any, index: number) => (
                    <tr key={item.id} className="bg-white dark:bg-gray-900">
                      ...
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300">{item?.po_no}</td>
                      <td className="py-3 px-4 text-gray-900 dark:text-gray-300 w-[500px]">
                        {item?.bill_to}
                      </td>
      ...
      <ComonDeletemodal
        ...
        title="Are you sure you want to Delete this Po Purchase?"
      />
...
export default PoPurchaseTable;
