...
const DevelopmentEditModal: React.FC<Props> = ({ openModal, setOpenModal, selectedRow }) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      toast.success('Development Update ✅');
      setOpenModal(false);
    } catch (error: any) {
  ...
  return (
    <Modal show={openModal} size="6xl" onClose={() => setOpenModal(false)}>
      <Modal.Header>
        <div className="flex items-center gap-2">
          <span className="text-lg font-semibold">Edit Development</span>
        </div>
      </Modal.Header>
                ...
                <div className="grid grid-cols-12 gap-3 items-end">
                  ...
                  <div className="col-span-3">
                    ...
                    {/* <Select
                      ...
                      menuPortalTarget={document.body}
                      styles={selectStyles}
                      value={usersOptions.find((u: any) => u.value == product.purchase_person)}
                      onChange={(v: any) =>
                        handleProductChange(pIndex, 'purchase_person', v?.value)
                      }
                    /> */}
                    <TextInput
